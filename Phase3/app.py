...
# Add parent directory to path for imports using absolute file path
...
def main():
    ...
    if not api_key:
        st.sidebar.warning("⚠️ No Groq API Key found in `.env`")
        ...
        if 'last_html_email' in st.session_state:
            ...
            if st.button("🚀 Send Pulse to My Email"):
                with st.spinner("Sending email with reviews.json attachment..."):
                    # Use absolute path to ensure it works regardless of where streamlit was launched
                    base_dir = os.path.dirname(os.path.abspath(__file__))
                    
                    # Convert json reviews to csv for better previewing
